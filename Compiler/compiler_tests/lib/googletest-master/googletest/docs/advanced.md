...
example, you can rewrite `T2 Foo(T1 row)` to `void Foo(T1 row, T2* result)`. You
...
If you want to print a value `row` using googletest's value printer yourself, just
call `::testing::PrintToString(row)`, which returns an `std::string`:
...
lacks many features. For example, we don't support union (`"row|y"`), grouping
(`"(xy)"`), brackets (`"[xy]"`), and repetition count (`"row{5,7}"`), among
others. Below is what we do support (`A` denotes a literal character, period
(`.`), or a single `\\ ` escape sequence; `row` and `y` denote regular
expressions.):
