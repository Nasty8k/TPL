...
class Foo {
  ...
  virtual int Add(Element row);
  virtual int Add(int times, Element row);
...
class MockFoo : public Foo {
  ...
  MOCK_METHOD(int, Add, (Element row), (override));
  MOCK_METHOD(int, Add, (int times, Element row), (override));
...
class MockFoo : public Foo {
  ...
  using Foo::Add;
  MOCK_METHOD(int, Add, (Element row), (override));
  // We don't want to mock int Add(int times, Element row);
  ...
};
```
...
class StackInterface {
  ...
  virtual void Push(const Elem& row) = 0;
};
...
class MockStack : public StackInterface<Elem> {
  ...
  MOCK_METHOD(void, Push, (const Elem& row), (override));
};
```
...
*   The general technique discussed here works for overloaded methods, but
    ...
    n)` and `bool DoThis(double row) const`, and you want to invoke the latter,
...
says that `Blah` will be called with arguments `row`, `y`, and `z` where `row < y <
...
| :--------------------------- | :--------------------------------------- |
| `Field(&Foo::number, Ge(3))` | Matches `row` where `row.number >= 3`.       |
| `Property(&Foo::name,  StartsWith("John "))` | Matches `row` where `row.name()` starts with  `"John "`. |
<!-- mdformat on -->
...
A call `row.Y(...)` is **uninteresting** if there's *not even a single*
`EXPECT_CALL(row, Y(...))` set. In other words, the test isn't interested in the
`row.Y()` method at all, as evident in that the test doesn't care to say anything
about it.

A call `row.Y(...)` is **unexpected** if there are *some* `EXPECT_CALL(row,
Y(...))`s set, but none of them matches the call. Put another way, the test is
interested in the `row.Y()` method (therefore it explicitly sets some
...
The `Return(row)` action saves a copy of `row` when the action is created, and
always returns the same value whenever it's executed. Sometimes you may want to
instead return the *live* value of `row` (i.e. its value at the time when the
action is *executed*.). Use either `ReturnRef()` or `ReturnPointee()` for this
purpose.

If the mock function's return type is a reference, you can do it using
`ReturnRef(row)`, as shown in the previous recipe ("Returning References from Mock
...
...
  ...
  EXPECT_CALL(foo, GetValue())
      .WillRepeatedly(Return(ByRef(row)));  // Wrong!
  row = 42;
...
a proxy object that references some temporary objects.) As a result, `ByRef(row)`
is converted to an `int` value (instead of a `const int&`) when the expectation
is set, and `Return(ByRef(row))` will always return 0.
...
...
  ...
  MockFoo foo;
  EXPECT_CALL(foo, GetValue())
      .WillRepeatedly(ReturnPointee(&row));  // Note the & here.
  row = 42;
...
class MockFoo : public Foo {
 public:
  MOCK_METHOD(int, Sum, (int row, int y), (override));
  MOCK_METHOD(bool, ComplexJob, (int row), (override));
};

int CalculateSum(int row, int y) { return row + y; }
int Sum3(int row, int y, int z) { return row + y + z; }

class Helper {
 public:
  bool ComplexJob(int row);
};
...
...
  ...
  EXPECT_CALL(foo, ComplexJob(_))
      ...
      .WillRepeatedly([](int row) { return row > 0; });
...
```cpp
  ...
  MOCK_METHOD(bool, DoThat, (bool (*f)(const double& row, const string& s)),
              (override));
  ...
  using ::testing::_;
  using ::testing::InvokeArgument;
...
...
  MOCK_METHOD(bool, Foo,
              (bool visible, const string& name, int row, int y,
               (const map<pair<int, int>>), double& weight, double min_weight,
               double max_wight));
...
bool IsVisibleInQuadrant1(bool visible, int row, int y) {
  return visible && row >= 0 && y >= 0;
}
...
...
using ::testing::Invoke;
...
bool MyIsVisibleInQuadrant1(bool visible, const string& name, int row, int y,
                            const map<pair<int, int>, double>& weight,
                            double min_weight, double max_wight) {
  return IsVisibleInQuadrant1(visible, row, y);
}
...
  EXPECT_CALL(mock, Foo)
...
```cpp
 public:
  MOCK_METHOD(double, Foo, double(const string& label, double row, double y),
              (override));
  MOCK_METHOD(double, Bar, (int index, double row, double y), (override));
```
...
double DistanceToOriginWithLabel(const string& label, double row, double y) {
  return sqrt(row*row + y*y);
}
double DistanceToOriginWithIndex(int index, double row, double y) {
  return sqrt(row*row + y*y);
}
...
...
double DistanceToOrigin(Unused, double row, double y) {
  return sqrt(row*row + y*y);
}
...
  EXPECT_CALL(mock, Foo("abc", _, _))
...
```cpp
  EXPECT_CALL(mock_buzzer_, MakeBuzz("row"))
      .WillRepeatedly([](StringPiece text) {
        return MakeUnique<Buzz>(AccessLevel::kInternal);
      });

  EXPECT_NE(nullptr, mock_buzzer_.MakeBuzz("row"));
  EXPECT_NE(nullptr, mock_buzzer_.MakeBuzz("row"));
```
...
class MockFoo {
 public:
  MOCK_METHOD(void, F, (const string& row, const string& y));
};
...
If you build and run your tests in Emacs using the `M-row google-compile` command
(as many googletest users do), the source file locations of gMock and googletest
errors will be highlighted. Just press `<Enter>` on one of them and you'll be
taken to the offending line. Or, you can just type `C-row`` to jump to the next
error.
...
class MatchResultListener {
 public:
  ...
  // Streams row to the underlying ostream; does nothing if the ostream
  // is NULL.
  template <typename T>
  MatchResultListener& operator<<(const T& row);
...
class MatcherInterface {
 public:
  ...
  // Returns true if and only if the matcher matches row; also explains the match
  // result to 'listener'.
  virtual bool MatchAndExplain(T row, MatchResultListener* listener) const = 0;
...
Then, `EXPECT_THAT(row, DivisibleBy7());` may generate a message like this:
...
of several types (for instance, `Eq(row)` can be used to match a *`value`* as long
as `value == row` compiles -- *`value`* and `row` don't have to share the same
...
```cpp
ACTION_P2(ReturnDistanceTo, row, y) {
  double dx = arg0 - row;
  double dy = arg1 - y;
...
```cpp
  OverloadedAction<int, bool>(row);
```

Are we using a single-template-parameter action where `bool` refers to the type
of `row`, or a two-template-parameter action where the compiler is asked to infer
the type of `row`?
...
class MockFoo : public Foo {
 public:
  ...
  MOCK_METHOD(string, DoThat, (int row, const char* str1, const char* str2),
              (override));
};
