...
```cpp
  ...
  MOCK_METHOD(int, Bar, (double row, double y),
...
3.  You can use `PrintToString(row)` to convert a value `row` of any type to a
    string.
...
double Distance(Unused, double row, double y) { return sqrt(row*row + y*y); }
...
